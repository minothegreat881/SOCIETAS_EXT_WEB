...
export default function ModernCalendar({ events, onEventClick, isAdmin = false }: ModernCalendarProps) {
  ...
  return (
    <Card className="w-full shadow-md border-0 overflow-hidden bg-white rounded-xl">
      ...
      <div className="relative">
        ...
        <CardContent className="p-4">
          <div className="mb-4 flex items-center justify-between">
            <Button variant="outline" size="icon" className="rounded-full h-7 w-7 border-stone-300" onClick={prevMonth}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <h2 className="text-base font-bold">{format(currentMonth, "LLLL yyyy", { locale: sk })}</h2>
            <Button variant="outline" size="icon" className="rounded-full h-7 w-7 border-stone-300" onClick={nextMonth}>
              <ChevronRight className="h-4 w-4" />
            </Button>
          </div>

          <div className="grid grid-cols-7 gap-1 text-center text-xs font-medium text-stone-500 mb-2">
            <div className="py-1">Po</div>
            <div className="py-1">Ut</div>
            <div className="py-1">St</div>
            <div className="py-1">Št</div>
            <div className="py-1">Pi</div>
            <div className="py-1">So</div>
            <div className="py-1">Ne</div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {allDays.map((day, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`min-h-[4rem] p-1 rounded-lg transition-all overflow-hidden ${
                    isSelected
                ...
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-center text-xs ${
                        isCurrentDay
                  ...
                  <div className="mt-1 space-y-1">
                    ...
                    {dayEvents.length > 2 && (
                      <div
                        className="text-xs text-stone-500 font-medium bg-stone-100 rounded px-1 py-0.5 hover:bg-stone-200 cursor-pointer"
                        onClick={(e) => {
                          e.stopPropagation()
                          if (dayEvents.length > 0 && onEventClick) {
                            onEventClick(dayEvents[0].id)
      ...
      <CardFooter className="pt-2 border-t flex justify-between bg-stone-50 px-4 py-2 text-xs">
        <div className="flex gap-2">
          <Badge variant="outline" className="bg-red-100 text-red-800 hover:bg-red-200 border-0 text-xs py-0">
            Tréning
          </Badge>
          <Badge variant="outline" className="bg-amber-100 text-amber-800 hover:bg-amber-200 border-0 text-xs py-0">
            Stretnutie
          </Badge>
        </div>
        <Button variant="link" size="sm" className="text-red-800 text-xs h-5 p-0">
          Zobraziť všetky
        </Button>
      </CardFooter>
